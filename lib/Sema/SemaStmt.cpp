...
unsigned Sema::GetLoopUnrollTupleSize(ExprResult *TupleExprResult) {
  QualType qt = TupleExprResult->get()->getType();
  const Type* t = qt.getTypePtr();
  const TemplateSpecializationType * tt = t->getAs<TemplateSpecializationType>();
  return tt->getNumArgs();
}

StmtResult Sema::ActOnLoopUnrollStmt(SourceLocation ForLoc,
                              SourceLocation LParenLoc, StmtResult DeclarationPart,//VarDecl *XVar,
                              ExprResult TupleExpr, SourceLocation RParenLoc,
                              StmtResult Body, SourceLocation LBraceLoc,
                              SourceLocation RBraceLoc) {
  if (TupleExpr.isInvalid())
    return StmtError();

  if (isa<NullStmt>(Body.get()))
    getCurCompoundScope().setHasEmptyLoopBodies();

  //StmtResult BodyRes = Actions.ActOnCompoundStmt(T.getOpenLocation(), CloseLoc,
  //                                 Stmts, isStmtExpr);

  //this->CurContext->FindVariable("get");
  //this->getStdNamespace();
  //llvm::outs() << "Before StdTupleResult";

  //LookupResult StdTupleResult(*this, &this->PP.getIdentifierTable().get("get"), ForLoc,
  //                            Sema::LookupNameKind::LookupTagName);

  //llvm::outs() << "StdTupleResult";

  //ClassTemplateDecl *StdTupleTemplate = StdTupleResult.getAsSingle<ClassTemplateDecl>();
  
  //llvm::outs() << "StdTupleTemplate";

  SmallVector<Stmt*, 8> CmpdStmt;
  //CmpdStmt.push_back(XVar);
  CmpdStmt.push_back(DeclarationPart.getAs<Stmt>());
  CmpdStmt.push_back(TupleExpr.getAs<Stmt>());
  CmpdStmt.push_back(Body.getAs<Stmt>());

  //return CompoundStmt::Create(Context, CmpdStmt, LBraceLoc, RBraceLoc);
  return ActOnCompoundStmt(LBraceLoc, RBraceLoc, CmpdStmt, false);
}
