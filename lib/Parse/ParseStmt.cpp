...
Retry:
  ...
  case tok::kw___clang_loop_unroll:
    return ParseLoopUnrollStatement(TrailingElseLoc);
...
StmtResult
Parser::ParseLoopUnrollStatement(SourceLocation *TrailingElseLoc) {
    assert(Tok.is(tok::kw___clang_loop_unroll) && "Not a loop-unroll stmt!");
    if(!getLangOpts().CPlusPlus)
        return StmtError(Diag(Tok, diag::err_compat_loop_unrolling));
    SourceLocation LoopUnrollLoc = ConsumeToken();
    Diag(Tok, diag::warn_just_random) << "Consumed __clan...";
    if (Tok.isNot(tok::l_paren)) {
    ...
    //BalancedDelimiterTracker BraceTracker(*this, tok::l_brace);
    //ColonProtectionRAIIObject ColonProtection(*this, ColonIsSacred);
    /*this part of the code deals with the loop body expansion. */
    //ParseScope LoopUnrollBodyScope(this, Scope::ScopeFlags::DeclScope |
    //                                     Scope::ScopeFlags::ControlScope,
    //                                     true, /*EnteredScope */
    //                                     Tok.is(tok::l_brace) /*BeforeCompoundStmt*/);

    //BraceTracker.consumeOpen();

    // Read the body statement
    StmtResult Body(ParseCompoundStatement(false, Scope::ScopeFlags::DeclScope |
                                                  Scope::ScopeFlags::ControlScope));
    //StmtResult Body(ParseStatement(TrailingElseLoc));

    llvm::outs() << Body.isInvalid() << "\n";; 

    //BraceTracker.consumeClose();

    // Pop the body scope if needed.
    //LoopUnrollBodyScope.Exit();

    // Leave the loop_unroll scope.
    LoopUnrollScope.Exit();
    ...
    //ActOnCallExpr()
    //Actions.ActOnCallExpr(nullptr, i, DeclarationPart.get().getLocEnd(), None, Context.DependentTy, VK_RValue, RParenLoc)
    //CallExpr GetCall = new CallExpr(Context, Fn, None, Context.VoidTy, VK_RValue, Body.get()->getLocStart())

    //Decl DGVar = DGPtr.get().getSingleDecl();
    //VarDecl *VD = llvm::dyn_cast<VarDecl>(DGVar);
    //VD->setInit(IntegerLiteral::Create(Context, llvm::APInt(32, 0), IntTy, ParenTracker.getCloseLocation()));

    //return StmtError();
    return Actions.ActOnLoopUnrollStmt(LoopUnrollLoc, ParenTracker.getOpenLocation(),
                         DeclarationPart, LUInit.TupleExpr,
                         ParenTracker.getCloseLocation(), Body,
                         Body.get()->getLocStart(), Body.get()->getLocEnd());
}
