...
Parser::DeclGroupPtrTy
Parser::ParseLoopUnrollDeclaration(LoopUnrollInit *LUInit,
                                   SourceLocation &DeclEnd,
                                   ParsedAttributesWithRange &Attributes) {
    ParsingDeclSpec PDS(*this);
    ParseDeclarationSpecifiers(PDS, ParsedTemplateInfo(),
                               AccessSpecifier::AS_none,
                               DeclSpecContext::DSC_normal);
    PDS.takeAttributesFrom(Attributes);
    ParsingDeclarator PD(*this, PDS, Declarator::TheContext::BlockContext);
    ParseDeclarator(PD);
    TryConsumeToken(tok::colon, LUInit->ColonLocation);
    if (Tok.is(tok::l_brace)) {
        Diag(Tok, diag::warn_just_random) << "Parsing brace";
        LUInit->TupleExpr = ParseBraceInitializer();
    } else {
        Diag(Tok, diag::warn_just_random) << "Parsing expr";
        LUInit->TupleExpr = ParseExpression();
        // QualType qt = LUInit->TupleExpr.get()->getType();
        // const Type* t = qt.getTypePtr();
        // const TemplateSpecializationType * tt = t->getAs<TemplateSpecializationType>();
        //TemplateSpecializationType* ts = llvm::dyn_cast<TemplateSpecializationType>(t);
        // Diag(Tok, diag::warn_just_random) << tt->getNumArgs();
        // Diag(Tok, diag::warn_just_random) << tt->getArg(0);
        //Diag(Tok, diag::warn_just_random) << t->getAsTagDecl()->getNumTemplateParameterLists(); //Sema::isSimpleTemplateIdType(qt);
        //dyn_cast<TemplateSpecializationType>(LUInit->TupleExpr.get()->getType().getTypePtr())->getAliasedType();//->template_arguments;//.getLocalUnqualifiedType();//.isInvalid();
    }
    Decl *LoopUnrollDecl = Actions.ActOnDeclarator(getCurScope(), PD);
    VarDecl *VD = llvm::dyn_cast<VarDecl>(LoopUnrollDecl);
    /* TODO:
     * Write code to check whether vardecl typecast was successful or not.
     */
    if (VD == nullptr) {
        Diag(Tok, diag::warn_just_random) << "Dyn cast null";
    }
    //VD->setInit(IntegerLiteral::Create(*this, llvm::APInt(32, 0), this->Context.IntTy, DeclEnd));
    Diag(Tok, diag::warn_just_random) << VD;
    VD->setCXXLoopUnrollDeclaration(true);
    /* the loop unroll declaration cannot have storage class specifiers
     * such as extern, static, auto or register, Emit suitable error for
     * that and set the invalid bit
     */
    Actions.FinalizeDeclaration(LoopUnrollDecl);
    PD.complete(LoopUnrollDecl);
    LUInit->ControlVarDecl = VD;
    return Actions.FinalizeDeclaratorGroup(getCurScope(), PDS, LoopUnrollDecl);
}
